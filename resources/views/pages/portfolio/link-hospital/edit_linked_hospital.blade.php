@extends('layout.main')
@section('title', "Edit Linked Portfolio With Hospital")
@section('content')
    <div class="page-container relative h-full flex flex-auto flex-col px-4 sm:px-6 md:px-8 py-4 sm:py-6">
        <div class="container mx-auto">
            <div class="flex justify-between items-center mb-4">
                <div class="flex items-center gap-4">
                    <a href="{{route('linked.hospital.list')}}">
            ...
            <div class="grid border border-gray-950 border-dashed rounded-md p-8 mt-5">
                <div>
                    ...
                    @if ($linked_hospital->status == 1)
                        <button class="btn bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 text-white update-account-status" type="button" data-url="{{ route('linked.hospital.update.status') }}" data-id="{{ encrypt($linked_hospital->id) }}" data-status=0>
                    ...
                    @else
                        <button class="btn btn-md bg-rose-600 hover:bg-rose-500 active:bg-rose-700 text-white update-account-status" type="button" data-url="{{ route('linked.hospital.update.status') }}" data-id="{{ encrypt($linked_hospital->id) }}" data-status=1>
                            <span class="flex items-center justify-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true" height="1em" width="1em" class="menu-item-icon">
        ...
        $(document).ready(function() {
            ...
            $('#editLinkedHospitalForm').on('submit', function(e){
                ...
                $.ajax({
                    ...
                    url:"{{ route('linked.hospital.edit.save') }}",
                    type:"POST",
                    processData: false,
                    contentType: false,
                    data:formData,
